...
**Deferred for v1 (2026-09-10): no quota at all.** This note is the v2 question, `Q_quota`; `solution.md`
→ *What v1 does not do* and `D_stats` both point here.
...
- A `post-app-create` / `pre-deploy` **plugin trigger** of our own that re-does the sum — moves the
  check off the `create-app` path, at the price of a plugin we maintain, which `AGENTS.md`'s
