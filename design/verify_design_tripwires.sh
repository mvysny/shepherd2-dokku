...
# --- cited slug → heading -----------------------------------------------------
# Shepherd2 is the third implementation of this product and deliberately cites the decisions of the
# first two by slug, with the repo named ("see D_no_shared_cache in shepherd-traefik"); those
# headings live in https://github.com/mvysny/shepherd-traefik/blob/main/DECISIONS.md and will never
# appear here. See design/decisions.md's preamble. Nothing else belongs on this list.
INHERITED='D_kubernetes|D_docker_traefik|D_network_per_project|D_poll_scm|D_no_shared_cache'
...
check_namespace() {
  ...
  cited=$(tracked | xargs -0 grep -I -o -h -E "$pattern" 2>/dev/null | grep -v -x -E "$INHERITED" | sort -u || true)
  if [ ! -f "$file" ]; then
